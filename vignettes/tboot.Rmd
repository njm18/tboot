...
author: "Will Landau, Nathan Morris"
date: "`r Sys.Date()`"
...
In the case of the Euclidian distance the problem is a standard "Quadratic Programing" problem which we solve using the 'ipop' function in the 'kernlap' package.
...
$$
and $S^*={q^*|\space \sum_{i = 1}^n q_i^* = 1}$. The weights ($W$) is a diagonal matrix with diagonal entries $W_{kk}=1/s^2_k$ where $s^2_k$ is the sample variance of column $k$ from the data matrix ($X$).

## Unable to find an optimum
For the KL dististance, the algorythm my fail to converge properly, and an error or warning will be issued.
