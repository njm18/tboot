...
## The ideal optimization problem
...
which is the same as minimizing $-\sum_{i = 1}^n \log q_i$. But this optimization problem has constraints:

1. $\sum_{i = 1}^n q_i = 1$ ($q$ is a probability distribution).
2. The expected column means from bootstrapping must reach the target: that is, $X'q = y$, where

    - $X'$ is the transpose of the original dataset $X$, and 
    - $y$ is the vector of target bootstrap column means.
...
&= \sum_{i = 1}^n \left [ -\log q_i + \lambda_0 \left ( q_i - \frac{1}{n} \right ) + \sum_{k = 1}^K \lambda_k (X_{ik} q_i - y_k) \right ]
\end{aligned}
$$
...
## The locally optimal resampling distribution
...
&= \frac{\partial}{\partial q_i} \sum_{i = 1}^n \left [ -\log q_i + \lambda_0 \left ( q_i - \frac{1}{n} \right ) + \sum_{k = 1}^K \lambda_k (X_{ik} q_i - y_k) \right ] \\
&= \frac{\partial}{\partial q_i} \left [ -\log q_i + \lambda_0 \left ( q_i - \frac{1}{n} \right )  + \sum_{k = 1}^K \lambda_k (X_{ik} q_i - y_k) \right ] \\
&= -\frac{1}{q_i} + \lambda_0 + \sum_{k = 1}^K \lambda_k X_{ik}  \\
...
In the numerical optimization, we should start from a uniform resampling distribution.

$$
\begin{aligned}
\frac{1}{n} &= q_i^*(\lambda) \\
&= \frac{1}{\sum_{k = 1}^K \lambda_k X_{ik}} \\
n &= \sum_{k = 1}^K \lambda_k X_{ik}
\end{aligned}
$$

Letting $J_{n \times 1}$ be the length-$n$ column vector of ones,

$$
\begin{aligned}
n J_{n \times 1} &= X \lambda
\end{aligned}
$$

This is a regression of $n J_{n \times 1}$ on $X$, which should have the solution 

$$
\begin{aligned}
\widehat{\lambda} &= \left (X'X \right )^{-} X' n J_{n \times 1}
\end{aligned}
$$

where $M^{-}$ is the generalized inverse of matrix $M$.
