...
2. Simulate the posterior mean as $\mu_k = Q(y_{k}, \Phi(Z_k))$
...
The algorithm described above may be justified based on...

#Comparing Bayesian Marginal Reconstruction with Full Multivariate Bayesian Analysis

##Example 1: Multivariate Normal 
```{r sim_trial, cache = FALSE}
#Manage any errors by assuming the pvalue failed to reach statistical #significance. Such errors here are due to the occasional occurance of
#
manageError=function(expr) tryCatch(eval(quote(expr)), error=function(e) return(1)) 
#create function to simulate and analyze one virtual trial
sim_and_analyze=function() {
